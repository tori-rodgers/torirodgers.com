...
async function getDevtoPost(slug) {
  // Fetch the post by slug using the Dev.to single-article endpoint
  const res = await fetch(`https://dev.to/api/articles/torirodgers/${slug}`, { next: { revalidate: 3600 } });
...
export default async function PostPage({ params }) {
  ...
  if (!post) return <div className="container mx-auto p-8 pt-24">Post not found.</div>;
  return (
    <div>
      <Header />
      <div className="container mx-auto p-8 pt-24 max-w-3xl">
        <div className="mb-6 flex flex-col sm:flex-row gap-6 items-center">
          {post.social_image && (
            <Image src={post.social_image} alt={post.title} width={200} height={200} className="rounded w-48 h-48 object-cover" unoptimized />
          )}
          <div>
            <h1 className="text-3xl font-bold mb-2">{post.title}</h1>
            <div className="text-gray-400 text-sm mb-2">{post.readable_publish_date || post.published_at?.split('T')[0]}</div>
            <div className="text-gray-300 text-base mb-2">{post.description}</div>
